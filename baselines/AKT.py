...
class AKT(nn.Module):
    ...
    def predict(self, q, s, pid=None, n=1, d_correct=None, d_skill_correct=None):
        q = q.masked_fill(q < 0, 0)
        q = q.long()  # 将 q 转换为 LongTensor
        s = s.masked_fill(s < 0, 0)
        s = s.long()  # 如果 s 也是传入 embedding 的，需要转换为 LongTensor
        ...
        if pid is not None:
            ...
            pid = pid.long()  # 将 pid 转换为 LongTensor
    ...
    def get_loss(self, q, s, pid=None, n=1, d_correct=None, d_skill_correct=None):
...
def attention(q, k, v, mask, gamma=None):
    ...
    if gamma is not None:
        ...
        with torch.no_grad():
            scores = scores.float()  # 将 scores 转换为 float32
