...
# 是否使用问题的ID\parser.add_argument("-p", "--with_pid", help="provide model with pid", action="store_true")

# 模型设置\parser.add_argument("-m", "--model", help="choose model", default="DCTKT")
...
def load_model(args, dataset):
    ...
    model = DCTKT(
        ...
        lambda_fcl=args.lambda_cl,  # 对比学习的损失权重参数，前向对比学习
        lambda_bcl=args.lambda_cl,  # 对比学习的损失权重参数，反向对比学习
